...
chcktxtnsuperr:
    ...
    cmp byte[stdinf], 255                   ;check if stdin was activated(its here for the case were there is no file input)
    je stdinmd5
...
loopfile:
    ...
    cmp byte[ebx], 2dh                      ;activate stdin for the detection of a -(only happens when it's just the -(2dh,0))
    je stdion
    jmp md5                                 ;jump to md5 label(it will be acting as a function)

stdion:
    mov byte[stdinf], 255                   ;activate the flag stdinf
    jmp stdinmd5
